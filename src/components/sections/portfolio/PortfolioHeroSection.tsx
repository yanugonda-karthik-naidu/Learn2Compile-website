...
function HeroScene({ mousePos }: { mousePos: { x: number; y: number } }) {
  ...
  const screenRef1 = useRef<THREE.Mesh | null>(null);
  const screenRef2 = useRef<THREE.Mesh | null>(null);
  const screenRef3 = useRef<THREE.Mesh | null>(null);

  useEffect(() => {
    if (!groupRef.current) return;
    groupRef.current.rotation.y = THREE.MathUtils.lerp(groupRef.current.rotation.y, mousePos.x * 0.15, 0.05);
    groupRef.current.rotation.x = THREE.MathUtils.lerp(groupRef.current.rotation.x, -mousePos.y * 0.1, 0.05);
  }, [mousePos]);

  useEffect(() => {
    const meshes = [screenRef1.current, screenRef2.current, screenRef3.current];
    meshes.forEach((mesh, i) => {
      if (!mesh) return;
      gsap.fromTo(mesh.scale, { x: 0, y: 0, z: 0 }, { x: 1, y: 1, z: 1, duration: 1.2, delay: 0.3 + i * 0.2, ease: "power3.out" });
    });
  }, []);
  ...
  return (
    <group ref={groupRef}>
      <Float speed={1.0} floatIntensity={0.25} rotationIntensity={0.15}>
        <mesh ref={screenRef1} position={[-1.4, 0.1, 0]} rotation={[0, 0.25, 0]}>
          <boxGeometry args={[1.1, 0.7, 0.02]} />
          <meshStandardMaterial color="#38BDF8" metalness={0.95} roughness={0.15} emissive="#38BDF8" emissiveIntensity={0.25} />
        </mesh>
      </Float>
      <Float speed={1.3} floatIntensity={0.35} rotationIntensity={0.2}>
        <mesh ref={screenRef2} position={[0, -0.1, 0.4]} rotation={[0, 0, 0]}>
          <boxGeometry args={[1.4, 0.9, 0.02]} />
          <meshStandardMaterial color="#8B5CF6" metalness={0.95} roughness={0.15} emissive="#8B5CF6" emissiveIntensity={0.25} />
        </mesh>
      </Float>
      <Float speed={0.9} floatIntensity={0.2} rotationIntensity={0.1}>
        <mesh ref={screenRef3} position={[1.5, 0.2, 0]} rotation={[0, -0.2, 0]}>
          <boxGeometry args={[0.9, 0.6, 0.02]} />
          <meshStandardMaterial color="#06B6D4" metalness={0.95} roughness={0.15} emissive="#06B6D4" emissiveIntensity={0.25} />
        </mesh>
      </Float>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h1 ref={titleRef} className="mt-6 text-4xl font-semibold leading-[1.08] tracking-tight sm:text-5xl lg:text-6xl">
              <span className="text-white">Premium Digital </span><br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#38BDF8] via-[#8B5CF6] to-[#06B6D4]">Experiences That Convert</span>
            </h1>
            <p ref={subtitleRef} className="mt-6 max-w-xl text-base leading-7 text-white/70 sm:text-lg">
              A curated collection of immersive builds that showcase modern design, conversion-focused engineering, and measurable business outcomes.
            </p>
            <div ref={ctaRef} className="mt-8 flex flex-wrap gap-4">
              <a href="#projects" className="group inline-flex items-center justify-center rounded-2xl border border-white/15 bg-white/5 px-6 py-3 text-sm font-medium text-white shadow-[0_0_0_1px_rgba(255,255,255,0.06)] transition-all duration-200 hover:border-[#38BDF8]/30 hover:bg-white/10 hover:shadow-[0_0_30px_rgba(56,189,248,0.2)]">
                <span>Explore Projects</span>
                <svg className="ml-2 h-4 w-4 transition-transform group-hover:translate-y-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg>
              </a>
