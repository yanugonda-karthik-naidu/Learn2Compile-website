...
export function Navbar() {
  ...
  useEffect(() => {
    if (!navbarRef.current) return;
    if (reduced) return;
    gsap.to(navbarRef.current, { y: isHidden ? -100 : 0, duration: motion.hover.duration.slow, ease: "power3.inOut" });
  }, [isHidden, reduced]);
          ...
          <div className={`flex h-14 sm:h-16 items-center justify-between transition-all duration-300 ${isScrolled ? "h-12 sm:h-14" : "h-14 sm:h-16"}`}>
            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => {
                ...
                return (
                  <Link key={link.href} href={link.href} aria-current={isActive ? "page" : undefined} className={`relative px-4 py-2 text-sm font-medium transition-all duration-200 rounded-xl group ${isActive ? "text-white" : "text-white/70 hover:text-white"}`}>
                    ...
                    {isActive && <span className="absolute -bottom-0.5 left-1/2 h-0.5 w-0 -translate-x-1/2 rounded-full bg-gradient-to-r from-[#38BDF8] via-[#8B5CF6] to-[#06B6D4] shadow-[0_0_8px_rgba(56,189,248,0.6)]" />}
                  </Link>
                );
              })}
            </div>
            ...
            <button onClick={() => setIsMobileOpen(true)} className="relative md:hidden flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white transition-all duration-200 hover:bg-white/10 hover:border-white/20 active:scale-95" aria-label="Open menu">
              <div className="flex flex-col gap-1.5">
                <span className={`h-0.5 w-5 rounded-full bg-white/80 transition-all duration-300 origin-center ${isMobileOpen ? "translate-y-2 rotate-45" : ""}`} />
