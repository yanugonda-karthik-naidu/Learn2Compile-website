...
export function MobileMenu({ isOpen, onClose, links }: MobileMenuProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      gsap.fromTo(linkElements, { x: 50, opacity: 0 }, { x: 0, opacity: 1, duration: 0.4, stagger: 0.05, ease: "power3.out", delay: 0.2 });
    } else {
      gsap.to(linkElements, { x: 50, opacity: 0, duration: 0.2, stagger: 0.03, ease: "power2.in" });
      gsap.to(menu, { x: "100%", duration: 0.4, ease: "power3.in", delay: 0.1 });
      gsap.to(overlay, { opacity: 0, duration: 0.3, ease: "power2.in", delay: 0.3, onComplete: () => {
  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm lg:hidden" style={{ display: "none" }} onClick={onClose}>
      <div ref={menuRef} className="fixed inset-y-0 right-0 z-50 flex w-full max-w-sm flex-col justify-center bg-[#050816]/95 backdrop-blur-xl border-l border-white/10" style={{ display: "none" }}>
        <div className="absolute right-4 top-4">
          <button onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white transition-all duration-200 hover:bg-white/10">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>

        <div ref={linksRef} className="flex flex-col gap-2 px-6">
          {links.map((link) => {
            const isActive = pathname === link.href || (link.href !== "/" && pathname.startsWith(link.href));
            return (
              <Link key={link.href} href={link.href} onClick={onClose} className={`menu-link group relative px-4 py-3 text-2xl font-medium transition-all duration-200 rounded-2xl ${isActive ? "text-white" : "text-white/70 hover:text-white"}`}>
          ...
          <div className="mt-6 menu-link">
            <Link href="/custom-quote" onClick={onClose} className="inline-flex w-full items-center justify-center rounded-2xl bg-gradient-to-r from-[#38BDF8]/20 via-[#8B5CF6]/20 to-[#06B6D4]/20 px-6 py-4 text-base font-medium text-white shadow-[0_0_30px_rgba(56,189,248,0.15)] transition-all duration-200 hover:from-[#38BDF8]/30 hover:via-[#8B5CF6]/30 hover:to-[#06B6D4]/30">
              Start Project
